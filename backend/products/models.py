...
class Product(models.Model):
    brand = models.ForeignKey(
        "products.Brand", on_delete=models.SET_NULL, blank=True, null=True
    )
    ...
    avg_rating = models.DecimalField(
        max_digits=2, decimal_places=1, default=0.0, blank=True, null=True
    )
    ...
    def get_star_ratings(self):
        ...
        ratings = self.ratings.values("rating").annotate(count=Count("id"))

        ratings_total = self.ratings.count()

        ratings_dict = {star: 0 for star in range(1, 6)}
        for entry in ratings:
            ratings_dict[entry["rating"]] = round(
                (entry["count"] / ratings_total) * 100
            )
...
class ProductAttributeValue(models.Model):
    product = models.ForeignKey(
        "products.Product", on_delete=models.CASCADE, related_name="attributes"
    )
    attribute = models.ForeignKey(
        "categories.CategoryAttribute", on_delete=models.CASCADE
    )
...
class ProductHighlight(models.Model):
    product = models.ForeignKey(
        Product, related_name="highlights", on_delete=models.CASCADE
    )
